...
function appWith(response: Response, canonicalCompanyId: string | null = "myci") {
  const app = express();
  ...
  app.use(myceliumProjectionRoutes(client, { canonicalCompanyId: async () => canonicalCompanyId }));
